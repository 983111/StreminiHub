...
// ---------------------------------------------------------------------------
// buildCompilableLatex
//
// main.tex produced by AppContext already has ALL section content inlined —
// there are NO \input{} commands in it. So we just return it as-is.
// For individual snippet files we wrap them in a minimal IEEE doc.
// We NEVER run a recursive regex over the result.
// ---------------------------------------------------------------------------
function buildCompilableLatex(
  ...
  // main.tex is already complete — return verbatim
  if (activeFile === 'main.tex') return content;

  // Snippet file: strip any accidental preamble lines the model added
  ...
  const safeFile = activeFile.replace(/_/g, '\\_');
...
// ---------------------------------------------------------------------------
// Compile via latex.ytotech.com — JSON body, returns PDF blob
// ---------------------------------------------------------------------------
...
export default function PaperGeneration() {
  ...
  const handleDownloadTex = () => {
    ...
    const url = URL.createObjectURL(new Blob([latexToCompile], { type: 'text/plain' }));
    Object.assign(document.createElement('a'), { href: url, download: 'paper.tex' }).click();
      ...
      <div className="w-72 border-r border-slate-200 bg-white flex flex-col shrink-0">
        ...
        {activeSidebarTab === 'tasks' && (
          <div className="flex-1 overflow-y-auto p-2 space-y-1">
            ...
            <TaskItem title="Assembly"            subtitle="Building final main.tex"     status={stageStatus('review',      generationStatus)} active={generationStatus === 'review'} />
        ...
        {activeSidebarTab === 'references' && (
          <div className="flex-1 overflow-y-auto p-3 space-y-3">
            ...
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">References</span>
      ...
      <div className="flex-1 flex min-w-0">
        ...
        <div className="w-48 border-r border-slate-200 bg-white flex flex-col shrink-0">
          ...
          <div className="p-1.5 space-y-0.5">
            {fileList.map(f => {
              const hasContent = !!activeWork?.generatedFiles?.[f.name];
              return (
                <div key={f.name} onClick={() => setActiveFile(f.name)}
                  className={'flex items-center gap-1.5 px-2 py-1.5 rounded cursor-pointer text-xs ' + (f.indent ? 'pl-5 ' : '') + (activeFile === f.name ? 'bg-indigo-50 text-indigo-600 font-medium' : 'text-slate-600 hover:bg-slate-50')}>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="h-11 border-b border-slate-200 flex items-center justify-between px-3 bg-slate-50 shrink-0">
            ...
            <div className="flex items-center gap-2">
              <button onClick={handleCompile} disabled={isCompiling || !latexToCompile}
              ...
              <div className="relative">
                ...
                {showDownload && (
                  <div className="absolute right-0 top-full mt-1 w-48 bg-white border border-slate-200 rounded-lg shadow-xl p-1.5 z-50">
                    <button onClick={handleDownloadTex} className="w-full text-left px-3 py-2 text-sm text-slate-700 hover:bg-slate-100 rounded-md">LaTeX source (.tex)</button>
                    <button onClick={handleDownloadPdf} className="w-full text-left px-3 py-2 text-sm text-slate-700 hover:bg-slate-100 rounded-md">Compiled PDF</button>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="flex-1 flex min-h-0">
            ...
            {(viewMode === 'pdf' || viewMode === 'split') && (
              <div className={'flex flex-col min-h-0 bg-slate-200 relative ' + (viewMode === 'split' ? 'w-1/2' : 'flex-1')}>
                ...
                {/* Error */}
                {compileError && (
                  <div className="m-3 p-3 bg-red-50 border border-red-200 rounded-lg text-xs text-red-700 flex gap-2 overflow-auto max-h-56 shrink-0">
                ...
                {isCompiling && (
                  <div className="absolute inset-0 bg-white/75 flex flex-col items-center justify-center z-10 gap-3">
                    ...
                    <p className="text-xs text-slate-400">10–25 seconds</p>
                  </div>
                )}
                ...
                {pdfUrl && !isCompiling ? (
                  ...
                  <div className="flex-1 flex items-center justify-center">
                    <div className="text-center text-slate-500">
                      ...
                      <p className="text-sm">Click <strong>Compile PDF</strong> to render</p>
                    </div>
                  </div>
                ) : null}
              </div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <div className="p-5 space-y-3">
              {[
                ...
                { label: 'Authors', k: 'authors', ph: 'e.g. Smith J., Doe J.' },
              ].map(({ label, k, ph }) => (
                <div key={k}>
